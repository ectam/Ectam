...
{
    QString ret = QString("Ectam:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ECT unit input in lnReqAmount, we generate the URI with ECT as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EctamUnits::format(EctamUnits::ECT, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
